...
void Core::renderImgui() {
	...
	ImGui::Begin("Interact");   // Pass a pointer to our bool variable (the window will have a closing button that will clear the bool when clicked)
	for(int k = 0; k < 6; k++)
	for (int i = 0; i < 6; i++)
	{
		if (i > 0)
		...
		if (ImGui::Button(gameplaySystem.cubeNotations[6 * k + i].c_str(), ImVec2(40, 25))) {
			gameplaySystem.rotateCubeSide(gameplaySystem.cubeNotations[6 * k + i].c_str());
		}
